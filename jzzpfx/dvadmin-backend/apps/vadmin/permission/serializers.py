...
from ..permission.models import Menu, Dept, Post, Role, UserProfile, Jzzp
...
class JzzpSerializer(CustomModelSerializer):
    """
    兼职招聘管理 简单序列化器
    """
...
class ExportJzzpSerializer(CustomModelSerializer):
    """
    导出 兼职招聘管理 简单序列化器
    """
    ...
    class Meta:
        model = Jzzp
        fields = ('id', 'zpzw', 'gsmc', 'xz', 'city', 'type', 'xl')


class JzzpSimpleSerializer(CustomModelSerializer):
    """
    兼职招聘管理 极简单序列化器
    """
...
class JzzpCreateUpdateSerializer(CustomModelSerializer):
    """
    兼职招聘管理 创建/更新时的列化器
    """
...
class UserProfileCreateUpdateSerializer(CustomModelSerializer):
    ...
    """
    admin = serializers.SerializerMethodField(read_only=True)
    post = JzzpSerializer(many=True, read_only=True)
